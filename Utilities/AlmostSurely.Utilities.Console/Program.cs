		...
		private static void Main(string[] args)
		{
			...
			using (var ms = new MemoryStream())
			{
				foreach(var imagePath in imagePaths)
				{
					...
					processContainer.Images.Add(imagePath, ms.ToArray());
				}
			}
			...
			File.WriteAllBytes(Path.Combine(options.OutputPath, "image.jpg"), processContainer.Images.First().Value.ToArray());
		}
	}
}