...
namespace AlmostSurely.Filters
{
    public class FirstFilter : BaseFilter
    {
		...
		{
			var imageOne = processContainer.Images.First();
			var magickImage = new MagickImage(imageOne.Value.ToArray());
			...
			processContainer.Images[imageOne.Key] = magickImage.ToByteArray();
